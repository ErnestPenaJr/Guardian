...
- Placeholder options in prototype: Account Takeover, Check Fraud, Identity Theft, Wire Fraud, Card Fraud, Phishing, Elder Fraud, Zelle / P2P Fraud, Business Email Compromise, Loan Fraud

### 1b. Category Dropdown *(NEW)*
- Add a second labeled dropdown in the same top row, after Fraud Type
- **Label:** `CATEGORY`
- **Options:** `[TBD — Sean to provide full list]`
- Placeholder options in prototype: Internal, External, Collusion, First Party, Third Party
...
- Within the label cell, include:
  - A **checkbox** to enable/activate code entry (`Enable code entry`)
  - When that checkbox is checked, reveal:
    - A **text input** for entering a restriction code
      - Maximum length: **4 characters**
      - Auto-uppercase on input
      - Enforce `maxlength="4"`
    - A **scrollable reference list** of existing codes (below the input)
      - There are thousands of codes — do **not** use a dropdown
      - Recommended pattern: scrollable container with autocomplete/typeahead against a data source
      - The enable checkbox controls visibility of the entire input + list panel
