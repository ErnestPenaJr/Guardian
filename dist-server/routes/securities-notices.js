...
function actorIds(req) {
    ...
    return {
        ...
        actorRoleId: req.actorRoleId ?? null,
    };
}
function buildTitle(format, fields) {
...
router.post('/', requireAuth, requireRole('securitiesNotice.send', 'send a securities notice'), async (req, res) => {
    try {
        ...
        const symbol = payload.fields.SECURITY_SYMBOL;
        if (!symbol || String(symbol).trim().length === 0) {
            return res.status(400).json({
                error: 'Security Symbol is a required field for Securities Fraud Notice templates and cannot be removed.',
            });
        }
        // Approval-required templates take the /submit path
        if (template.REQUIRES_MANAGER_APPROVAL) {
            return forbid(res, 'send a notice that requires manager approval — submit it for approval instead');
        }
        ...
        const title = buildTitle(template.TITLE_FORMAT, payload.fields);
...
router.put('/:id/submit', requireAuth, requireRole('securitiesNotice.submit', 'submit a notice for approval'), async (req, res) => {
    try {
        ...
        const processorName = [processor?.FIRST_NAME, processor?.LAST_NAME].filter(Boolean).join(' ') ||
            processor?.EMAIL ||
            `User #${userId}`;
        if (companyId != null) {
...
router.put('/:id/approve', requireAuth, requireRole('securitiesNotice.approve', 'approve notices'), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isFinite(id))
            return res.status(400).json({ error: 'Invalid notice id' });
...
router.put('/:id/reject', requireAuth, requireRole('securitiesNotice.approve', 'reject notices'), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isFinite(id))
            return res.status(400).json({ error: 'Invalid notice id' });
...
router.put('/:id/records-released', requireAuth, requireRole('securitiesNotice.markRecordsReleased', 'mark records released'), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isFinite(id))
            return res.status(400).json({ error: 'Invalid notice id' });
...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        const isGeneralUserOnly = userRoles.length === 1 && userRoles[0].ROLE_ID === 2; // GENERAL_USER only
