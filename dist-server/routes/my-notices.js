...
router.get("/export-csv", requireAuth, async (req, res) => {
    try {
        ...
        const data = notices.map((notice) => {
            const totalAttachments = notice.RESPONSES.reduce((count, resp) => (resp.ATTACHMENT ? count + 1 : count), 0);
            return {
        ...
        const TOTAL_ATTACHMENTS = data.reduce((sum, row) => sum + row.Attachments, 0);
        // Add totals row
        data.push({
            Notice: "TOTAL",
            Sensitivity: "",
...
router.get("/export-csv/:id", requireAuth, async (req, res) => {
    try {
        ...
        const responseRows = notice.RESPONSES.length > 0
...
router.get("/", requireAuth, async (req, res) => {
    try {
        const { search, status, sensitivity, distribution, page = "1", limit = "10", } = req.query;
...
router.get("/:id", requireAuth, async (req, res) => {
    try {
        ...
        if (!notice)
            return res.status(404).json({ error: "Notice not found" });
        if (notice.COMPANY_ID !== req.user.COMPANY_ID)
            return res.status(403).json({ error: "Access denied" });
...
router.post("/", requireAuth, canCreateNotice, async (req, res) => {
    try {
        ...
        const noticeCompanyId = req.user.COMPANY_ID;
...
router.put("/:id", requireAuth, canCreateNotice, async (req, res) => {
    const { id } = req.params;
    try {
        const data = NoticeSchema.parse(req.body);
        const putCompanyId = req.user.COMPANY_ID;
        const existingNoticeForUpdate = await prisma.mY_NOTICES.findUnique({
            where: { NOTICE_ID: parseInt(id) },
            select: { COMPANY_ID: true },
        });
        if (!existingNoticeForUpdate)
            return res.status(404).json({ error: "Notice not found" });
        if (existingNoticeForUpdate.COMPANY_ID !== putCompanyId)
            return res.status(403).json({ error: "Access denied" });
...
router.patch("/:id", requireAuth, upload.array("attachment"), async (req, res) => {
    ...
    try {
        const patchCompanyId = req.user.COMPANY_ID;
        const patchUserId = req.user.id;
        ...
        if (!recipientRecord)
            return res.status(403).json({ error: "You are not a recipient of this notice" });
