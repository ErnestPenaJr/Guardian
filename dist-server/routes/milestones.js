...
router.get('/requests/:requestId/milestones', requireAuth, async (req, res) => {
    ...
    `;
        ...
        if (progressTypes) {
            ...
            const safeTypes = rawTypes.filter((t) => typeof t === 'string' && VALID_PROGRESS_TYPES.includes(t));
...
router.put('/milestones/:milestoneId', requireAuth, async (req, res) => {
    ...
    `;
        ...
        const updateFields = [];
        if (title !== undefined)
            updateFields.push(`"TITLE" = '${title.replace(/'/g, "''")}'`);
        if (description !== undefined)
            updateFields.push(`"DESCRIPTION" = '${description.replace(/'/g, "''")}'`);
        if (isVisibleToRequestor !== undefined)
            updateFields.push(`"IS_VISIBLE_TO_REQUESTOR" = ${isVisibleToRequestor ? 'true' : 'false'}`);
        if (hoursWorked !== undefined) {
            const safeHours = parseFloat(hoursWorked);
            if (!isNaN(safeHours))
                updateFields.push(`"HOURS_WORKED" = ${safeHours}`);
        }
