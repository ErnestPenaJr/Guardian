...
router.post('/workflow/process', isProcessor, async (req, res) => {
    try {
        ...
        if (request.COMPANY_ID !== userCompanyId && req.user?.role !== '1') {
    ...
    `;
        if (nextStep[0]) {
            ...
            await prisma.tASKS.create({
                data: {
                    ...
                    STATUS: 'P',
                    ASSIGNED_USER_ID: nextStepData.ASSIGNED_USER_ID || req.user?.id || null,
        ...
        res.json({
            ...
            isComplete: remainingSteps[0].remaining === 0
        });
    }
    catch (error) {
        ...
        if (firstStep[0]) {
            ...
            await prisma.tASKS.create({
                data: {
                    ...
                    STATUS: 'P',
                    ASSIGNED_USER_ID: stepData.ASSIGNED_USER_ID || req.user?.id || null,
                    CREATE_USER_ID: req.user?.id || null,
                    UPDATE_USER_ID: req.user?.id || null,
                    CREATE_DATE: new Date(),
                    UPDATE_DATE: new Date()
...
router.put('/tasks/:id/complete', isProcessor, async (req, res) => {
    try {
        ...
        const userCompanyId = req.user?.COMPANY_ID;
        // Using type assertion since COMPANY_ID is not in the Prisma model but exists at runtime
        if (request && request.COMPANY_ID !== userCompanyId && req.user?.role !== '1') {
            return res.status(403).json({
