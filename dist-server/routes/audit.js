...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        if (f.from)
            where.CREATED_AT = { ...(where.CREATED_AT ?? {}), gte: new Date(f.from) };
        if (f.to)
            where.CREATED_AT = { ...(where.CREATED_AT ?? {}), lte: new Date(f.to) };
        if (f.eventType)
            where.EVENT_TYPE = f.eventType;
        if (f.actorRoleId)
            where.ACTOR_ROLE_ID = f.actorRoleId;
        if (f.actorUserId)
            where.ACTOR_USER_ID = f.actorUserId;
        if (f.targetId)
...
router.get('/export', requireAuth, requireRole('audit.export', 'export the audit log'), async (req, res) => {
    try {
        ...
        if (format === 'csv') {
            ...
            const csv = parser.parse(serialized.map((r) => ({ ...r, EVENT_DETAIL: r.EVENT_DETAIL ?? '' })));
        ...
        if (format === 'pdf') {
            ...
            serialized.forEach((r) => {
                doc
                    .fontSize(9)
                    .text(`${r.CREATED_AT}  ${r.EVENT_TYPE}  actor=${r.ACTOR_USER_ID ?? '—'} (role ${r.ACTOR_ROLE_ID ?? '—'})  target=${r.TARGET_TYPE}:${r.TARGET_ID ?? '—'}`);
                if (r.EVENT_DETAIL) {
