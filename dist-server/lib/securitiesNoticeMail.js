 ...
 */
export async function notifyManagersOfPending(companyId, noticeId, processorName) {
    try {
        ...
        const emails = managers.map((m) => m.EMAIL).filter((e) => Boolean(e));
        if (emails.length === 0) {
            console.log(`[SECURITIES NOTICE MAIL] No active managers found for company ${companyId}.`);
            return;
        }
        ...
        await resend.emails.send({
            ...
            text: `A Securities Fraud Notice submitted by ${processorName} is pending your approval.\n\n` +
      ...
      `,
        ...
        console.log(`[SECURITIES NOTICE MAIL] notifyManagersOfPending → ${emails.length} manager(s) for notice ${noticeId}`);
    }
    catch (err) {
 ...
 */
export async function notifyProcessorOfRejection(processorUserId, noticeId, reason) {
    try {
        ...
        if (!user?.EMAIL) {
            console.log(`[SECURITIES NOTICE MAIL] notifyProcessorOfRejection — no email for user ${processorUserId}.`);
            return;
        }
        ...
        await resend.emails.send({
            ...
            text: `Your Securities Fraud Notice (ID ${noticeId}) was returned for revision by a manager.\n\n` +
                `Reason: ${reason}\n\n` +
      ...
      `,
        ...
        console.log(`[SECURITIES NOTICE MAIL] notifyProcessorOfRejection → user ${processorUserId} for notice ${noticeId}`);
    }
    catch (err) {
