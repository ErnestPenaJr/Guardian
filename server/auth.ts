        ...
        try {
          ...
          if (testUser && isTestUser(testUser)) {
            ...
            const authenticatedUser: AuthUser = {
              ...
              role: (testUser as TestUser).roles.includes(1) ? '1' : (testUser as TestUser).roles.includes(6) ? '6' : (testUser as TestUser).roles.includes(5) ? '5' : (testUser as TestUser).roles[0]?.toString() || 'user', // Set role as string ID
            };
            return done(null, authenticatedUser);
          }
