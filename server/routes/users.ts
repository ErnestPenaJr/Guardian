...
router.post("/", requireAuth, isAdmin, async (req, res) => {
  try {
    ...
    console.log(
      `[ADD USER] Generated temporary password for ${email}: ${tempPassword}`,
    );
    ...
    try {
      ...
      if (resend) {
        ...
        console.log(
          "[ADD USER] Email not sent - Resend API key not configured",
        );
        console.log(
          "[ADD USER] Would have sent temporary password:",
          tempPassword,
        );
      }
    } catch (emailError) {
...
router.get(
  ...
  async (req: any, res) => {
    try {
      ...
      console.log(
        `[USERS] Fetching assignable users for company ID: ${companyId}`,
      );
...
router.get("/", isAdmin, async (req: any, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `[USERS] Found ${users.length} active users for company ID: ${companyId}`,
      );
    } catch (error) {
      ...
      try {
        ...
        console.log(
          "Fallback: returning all users due to company filtering error",
        );
      } catch (fallbackError) {
    ...
    const usersWithRoles = await Promise.all(
      formattedUsers.map(async (user) => {
        try {
          ...
          console.log(
            `[USERS] User ${user.id} (${user.firstName} ${user.lastName}) all roles:`,
            allUserRoles,
          );
          ...
          console.log(
            `[USERS] User ${user.id} (${user.firstName} ${user.lastName}) has ${userRoles.length} active roles:`,
          ...
          if (roleIds.length > 0) {
            try {
              ...
              console.error(
                `Error fetching roles from ROLES table for user ${user.id}:`,
                roleError,
              );
...
router.put("/:id", isAdmin, async (req: any, res) => {
  try {
    ...
    console.log(`[UPDATE USER] Updating user ${userId} with data:`, {
      firstName,
      lastName,
      email,
      roleId,
...
router.get("/all-users", isAdmin, async (req: any, res) => {
  try {
    const users = await prisma.uSERS.findMany({
