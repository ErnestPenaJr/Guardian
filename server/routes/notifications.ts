...
router.get('/', requireAuth, async (req: any, res) => {
  try {
    ...
    const data = await prisma.$queryRawUnsafe(
      `SELECT "NOTIFICATION_ID", "TYPE", "TITLE", "MESSAGE", "RELATED_ID", "IS_READ", "CREATED_DATE"
...
router.get('/count', requireAuth, async (req: any, res) => {
  try {
    const userId = req.user.id;
    const companyId = req.user.COMPANY_ID;
    if (companyId == null) {
    ...
    const result = await prisma.$queryRaw<{ unread_count: number }[]>`
...
router.put('/:notificationId/unread', requireAuth, async (req: any, res) => {
  try {
    const userId = req.user.id;
    const companyId = req.user.COMPANY_ID;
    const notificationId = parseInt(req.params.notificationId, 10);
