...
router.get('/purge/user/:userId/preview', async (req: Request, res: Response) => {
  try {
...
router.post('/purge/user/:userId', async (req: Request, res: Response) => {
  try {
    const userId = Number(req.params.userId)
...
router.get('/purge/company/:companyId/preview', async (req: Request, res: Response) => {
  try {
...
router.post('/purge/company/:companyId', async (req: Request, res: Response) => {
  try {
    const companyId = Number(req.params.companyId)
    const confirmation = String(req.body?.confirmation || '')
    if (!Number.isInteger(companyId) || companyId <= 0) {
