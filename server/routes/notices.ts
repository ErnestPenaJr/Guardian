...
router.get('/my', passport.authenticate('jwt', { session: false }), async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/stats', passport.authenticate('jwt', { session: false }), async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id || req.user?.USER_ID;
    ...
    const stats = {
      totalNotices: parseInt((totalNoticesResult as any[])[0]?.total_count) || 0,
      unreadNotices: parseInt((unreadNoticesResult as any[])[0]?.unread_count) || 0,
      issuedByMe: parseInt((issuedByMeResult as any[])[0]?.issued_count) || 0,
      activeNotices: parseInt((activeNoticesResult as any[])[0]?.active_count) || 0,
    };
...
router.get('/active', passport.authenticate('jwt', { session: false }), async (req: AuthRequest, res: Response) => {
  try {
