...
import myNotices from "./routes/my-notices.js";
...
async function captureServerError(
  ...
  try {
    console.log(
      `[SERVER ERROR CAPTURE] ${error.message} at ${context.endpoint}`,
    );
...
<body>
  <div class="container">
    ...
    console.log(
      `[SERVER ERROR CAPTURE] Error report sent for: ${context.endpoint}`,
    );
  } catch (emailError) {
    console.error(
      "[SERVER ERROR CAPTURE] Failed to send error email:",
      emailError,
    );
  }
}
...
app.use("/api", (req, res, next) => {
  console.log(
    `[API REQUEST] ${req.method} ${req.path} - User-Agent: ${req.get("User-Agent")}`,
  );
...
app.use("/api/my-notices", myNotices); // my notices
...
app.post("/api/login", loginRateLimiter, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "[LOGIN] Database connection status:",
      ...
      const user =
        (exactMatch as any[]).length > 0 ? (exactMatch as any[])[0] : null;
      ...
      if (user.COMPANY_ID) {
        try {
          console.log(
            `[LOGIN] Fetching company info for ID: ${user.COMPANY_ID}`,
          );
      ...
      if (
        ...
        console.error(
          "[LOGIN] Database connection error - check database configuration",
        );
    ...
    res.status(500).json({
      ...
      details:
        process.env.NODE_ENV === "development" ? outerErr.message : undefined,
    });
  }
});
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    console.log(
      "%c Company Name from Email Domain",
    ...
    try {
      ...
      console.warn(
        "%c [COMPANY ASSIGNMENT] Using company:",
  ...
  try {
    ...
    if (!user) {
      return res.status(404).json({
        error: "No user found for this email. Please register again.",
      });
    }
    ...
    const tokenExpiry =
      user.EMAIL_VALIDATION_TOKEN_EXPIRY ||
      new Date(new Date(user.UPDATE_DATE).getTime() + 1000 * 60 * 15);
  ...
  try {
    ...
    if (purpose === "register") {
      if (existingUser) {
        return res
          .status(409)
          .json({ valid: false, reason: "Email already registered" });
      }
      ...
      if (!existingUser) {
        return res
          .status(404)
          .json({ valid: false, reason: "Email not found" });
      }
      if (existingUser.EMAIL_VALIDATED !== true) {
        return res
          .status(403)
          .json({ valid: false, reason: "Email not verified" });
      }
      return res.json({ valid: true });
    } else {
    ...
    return res
      .status(500)
      .json({ error: "Server error during email validation" });
  }
});
...
app.post("/api/verify-email", async (req, res) => {
  try {
    ...
    if (!/^\d{6}$/.test(verificationCode)) {
      return res
        .status(400)
        .json({ error: "Verification code must be a 6-digit number" });
    }
...
app.post("/api/complete-registration", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Server error completing registration" });
  }
});
...
app.post("/api/request-password-reset", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Server error requesting password reset" });
  }
});
...
app.post(
  "/api/test/create-sample-requests",
  passport.authenticate("jwt", { session: false }),
  async (req: any, res) => {
    try {
      ...
      if (companyUsers.length === 0) {
        return res
          .status(404)
          .json({ error: "No users found in your company" });
      }
      ...
      for (let i = 0; i < 10; i++) {
        const requestorId =
          companyUsers[Math.floor(Math.random() * companyUsers.length)].USER_ID;
        const assignedId =
          Math.random() > 0.3
        ...
        try {
          await prisma.rEQUESTS.create({
            data: {
              ...
              EXTERNAL_USER:
                Math.random() > 0.5 ? `external${i}@example.com` : null,
      ...
      if (error instanceof Error) {
        ...
        res
          .status(500)
          .json({ error: "Failed to create sample requests", detail: error });
      }
    }
  },
);
...
app.listen(PORT, () => {
  ...
  console.log(
    `🚨 Global error capture system is active - errors will be emailed to ernest@shieldlytics.com`,
  );
});
