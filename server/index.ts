...
app.post('/api/complete-registration', async (req, res) => {
  try {
    ...
    // Hash password
    const hash = await bcrypt.hash(password, 10);
    // Always create a new company, regardless of name
    const company = await prisma.cOMPANY.create({ data: { NAME: workspaceName } });
    console.log('[COMPLETE REGISTRATION] created new company:', company);
