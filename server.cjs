...
// Azure SQL Serverless can take 30–90s to resume from auto-pause; during
// that window Prisma raises P1001/P1002/P1008/P1017 and the underlying
// driver surfaces ETIMEDOUT / ECONNREFUSED / "Connection lost" messages.
...
const getAuthenticatedUserCompany = async (req, res, next) => {
    try {
        ...
        // JAFAR impersonation: allow role-6 users to act AS any user in any
        // company by sending an X-Jafar-Impersonate-User-Id header. The
        // impersonator's identity is stashed on req.jafarImpersonator* for
        // future audit; req.userId / req.companyId / req.userRoleIds / req.user
        // are rewritten to the target user so all downstream permission
        // checks, scoping, and writes follow as if the target user did them.
...
async function __riderBuildIdentifierBlock(noticeId, companyId) {
    ...
    // Try the full SELECT including IS_PII (added by migration
    // 04_extend_forms_template_subtype.sql). If the column doesn't exist on
    // this env (migration not yet applied), fall back to the same query
    // without IS_PII — label-heuristic + content-scan still cover redaction.
    let fields;
    try {
...
app.post('/api/templates/subpoena', getAuthenticatedUserCompany, requirePermission('subpoenaRider.configureLanguage'), async (req, res) => {
    try {
        ...
        // Upsert via DELETE+INSERT. UQ_SUBPOENA_LANG_FRAUD_PER_COMPANY guards uniqueness.
        // Tagged-template raws so BASE_LANGUAGE/TOKENS_JSON are properly parameterized
        // (they're free text — string-concat would be a SQL-injection vector).
        await prisma.$executeRaw`
...
app.get('/api/custom-templates/:id', getAuthenticatedUserCompany, async (req, res) => {
    try {
        ...
        // Non-admins can read active notice templates (needed to render the Create Notice form).
        // Drafts + request templates remain admin-only.
...
app.post('/api/my-notices', getAuthenticatedUserCompany, async (req, res) => {
    try {
        ...
        // Expand contact groups → active member user IDs (same-company only)
...
app.get('/api/my-notices/:id', getAuthenticatedUserCompany, async (req, res) => {
    try {
        ...
        // Get recipients from MY_NOTICE_RECIPIENTS
        const recipients = await prisma.$queryRawUnsafe(`
            SELECT nr.NOTICE_RECIPIENT_ID, nr.NOTICE_ID, nr.USER_ID,
                   u.FIRST_NAME, u.LAST_NAME, u.EMAIL
            FROM GUARDIAN.MY_NOTICE_RECIPIENTS nr
            LEFT JOIN GUARDIAN.USERS u ON nr.USER_ID = u.USER_ID
        ...
        const notice = {
            ...
            RESPONSES: responses.map(r => ({
                RESPONSE_MY_NOTICE_ID: r.RESPONSE_MY_NOTICE_ID,
...
// PUT /api/my-notices/:id — update a draft notice
app.put('/api/my-notices/:id', getAuthenticatedUserCompany, async (req, res) => {
    try {
        const noticeId = parseInt(req.params.id, 10);
        if (Number.isNaN(noticeId)) {
            return res.status(400).json({ error: 'Invalid notice ID' });
        ...
        const existing = await prisma.$queryRawUnsafe(`
            SELECT NOTICE_ID, COMPANY_ID FROM GUARDIAN.MY_NOTICES WHERE NOTICE_ID = ${noticeId} AND COMPANY_ID = ${companyId}
        `);
        ...
        await prisma.$queryRawUnsafe(`
            ...
            SET NOTICE_TITLE = '${String(NOTICE_TITLE).replace(/'/g, "''")}',
                SENSITIVITY_CLASSIFICATION = '${String(SENSITIVITY_CLASSIFICATION).replace(/'/g, "''")}',
                BUTTON_STATUS = '${String(BUTTON_STATUS).replace(/'/g, "''")}',
                DISTRIBUTION_TYPE = '${String(DISTRIBUTION_TYPE || '').replace(/'/g, "''")}',
                NOTICE_BODY = '${String(NOTICE_BODY || '').replace(/'/g, "''")}',
                UPDATE_USER_ID = ${parseInt(req.userId, 10)},
                UPDATE_DATE = GETDATE()
        ...
        res.json({ message: 'Notice updated successfully', NOTICE_ID: noticeId });
    } catch (error) {
...
app.delete('/api/my-notices/:id', getAuthenticatedUserCompany, async (req, res) => {
    try {
        ...
        // Check notice exists and belongs to company
        const notices = await prisma.$queryRawUnsafe(`
        ...
        // Delete recipients, responses, then notice
        await prisma.$queryRawUnsafe(`DELETE FROM GUARDIAN.MY_NOTICE_RECIPIENTS WHERE NOTICE_ID = ${noticeId}`);
...
app.get('/api/jafar-admin/companies', getAuthenticatedUserCompany, checkJafarRole, async (req, res) => {
    try {
        ...
        // Only return companies with at least one active user — impersonation
        // requires an existing user to act as, so empty companies are useless
        // in the switcher dropdown.
...
// impersonation picker. Active = USERS.STATUS = 'A'. Returns role ids so the
// frontend can show role labels next to each candidate.
...
app.put('/api/jafar-admin/jafar-users/:userId/roles', getAuthenticatedUserCompany, checkJafarRole, async (req, res) => {
    try {
        ...
        // Must always include role 6 (Jafar) — can't remove Jafar role from this page
