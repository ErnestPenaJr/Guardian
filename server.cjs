...
// Send error email notification to system administrators
app.post('/api/send-error-email', async (req, res) => {
    try {
        const { to, subject, errorDetails, htmlBody } = req.body;
        console.log(`🚨 Error email request received - Subject: ${subject}`);
        
        if (!to || !subject || !errorDetails || !htmlBody) {
            return res.status(400).json({
                error: 'Email recipient, subject, error details, and HTML body are required'
            });
        }

        // Enhanced email validation for recipient
        const emailValidation = validateEmailServer(to);
        if (!emailValidation.valid) {
            console.log(`❌ Invalid recipient email format: ${to}`);
            return res.status(400).json({
        ...
        // Send error notification email using Resend
        try {
            console.log(`📧 Sending error notification to: ${normalizedEmail}`);
            ...
            if (error) {
                console.error('❌ Resend error sending error notification:', error);
                return res.status(500).json({
            ...
            console.log(`✅ Error notification email sent successfully to ${normalizedEmail}`);
        ...
        console.error('❌ Send error email endpoint error:', error);
        res.status(500).json({
            error: 'Internal server error processing error email request',
            message: error.message,
            timestamp: new Date().toISOString()
        });
    }
