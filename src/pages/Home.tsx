...
import Tabs from "../components/NoticeManagementSystem/Tabs";
...
function Home() {
  ...
  const isMyNoticesPath = location.pathname.startsWith("/my-notices");

  // Sync sidebar selection when URL is /my-notices*
  useEffect(() => {
    if (isMyNoticesPath) setSelectedSection("noticeManagementSystem");
  }, [isMyNoticesPath]);
  ...
  const checkAccountCreatorInviteModal = async () => {
    if (hasCheckedAccountCreatorInvite) {
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🎯 ===== SKIPPING ACCOUNT CREATOR INVITE CHECK - ALREADY CHECKED =====",
        );
        console.log(
          "🎯 hasCheckedAccountCreatorInvite:",
          hasCheckedAccountCreatorInvite,
        );
      }
      return;
    ...
    if (!isAdmin()) {
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🎯 User is not admin, skipping account creator invite check",
        );
      }
    ...
    if (
      user?.accountCreatorInviteCompleted ||
      user?.ACCOUNT_CREATOR_INVITE_COMPLETED
    ) {
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🎯 User has already completed account creator invite modal",
        );
      }
      setHasCheckedAccountCreatorInvite(true);
      return;
    ...
    try {
      const companyUsers = await api.get(
        `/api/users/company/${user?.companyId}`,
      );
      ...
      if (!isAccountCreator) {
        if (process.env.NODE_ENV === "development") {
          console.log(
            "🎯 User is not the account creator, skipping invite modal",
          );
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.log(
          "🎯 User is account creator and hasn't completed invite modal",
        );
      }
  ...
  const checkForExistingTemplates = async () => {
    if (hasCheckedForExistingTemplates) {
      if (process.env.NODE_ENV === "development") {
        ...
        console.log(
          "🔍 hasCheckedForExistingTemplates:",
          hasCheckedForExistingTemplates,
        );
      }
      return;
    ...
    try {
      ...
      if (!isAccountCreator) {
        if (process.env.NODE_ENV === "development") {
          console.log(
            "🔍 User is not the account creator, skipping template check",
          );
          console.log("🔍 First user in company:", companyUsers?.data?.[0]);
          console.log("🔍 Current user ID:", user?.id);
        }
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🔍 Could not verify account creator status, proceeding with template check",
        );
      }
    ...
    try {
      ...
      if (!companyForms || companyForms.length === 0) {
        if (process.env.NODE_ENV === "development") {
          ...
          console.log(
            "🚨 No existing company templates found, showing first-time workflow creation modal",
          );
        ...
        if (process.env.NODE_ENV === "development") {
          setTimeout(() => {
            console.log(
              "🚨 Modal state after setting:",
              showFirstTimeWorkflowModal,
            );
          }, 100);
        }
      } else {
  ...
  const debugToggleModal = () => {
    if (process.env.NODE_ENV === "development") {
      ...
      console.log(
        "🔧 Current showFirstTimeWorkflowModal:",
        showFirstTimeWorkflowModal,
      );
      ...
      try {
        ...
        if (forms && forms.length > 0) {
          ...
          forms.forEach((form, index) => {
            console.log(
              `🔧 Form ${index + 1}: "${form.FORM_NAME}" - ID: ${form.FORM_ID} - Company: ${form.COMPANY_ID}`,
            );
          });
        }
  ...
  useEffect(() => {
    if (location.state && location.state.activeSection) {
      const section = location.state.activeSection as typeof selectedSection;
      setSelectedSection(section);
      // If restoring a non–My Notices section while on /my-notices, go to /home so URL and sidebar stay in sync
      if (
        section !== "noticeManagementSystem" &&
  ...
  useEffect(() => {
    const unsubscribe = subscribeToRefresh(() => {
      if (process.env.NODE_ENV === "development") {
        console.log(
          "🔄 Global state refresh triggered - updating Home.tsx data",
        );
      }
      fetchRequests();
      fetchAssignedRequestsCount();
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === "development") {
      console.log(
        "🔄 ===== USE EFFECT FOR ACCOUNT CREATOR AND TEMPLATE CHECKS =====",
      );
  ...
  const handleSendInvite = () => {
    MySwal.fire({
      html: <SendInvitesForm onClose={() => MySwal.close()} />,
      showConfirmButton: false,
      showCancelButton: false,
      customClass: {
        popup:
          "p-0 bg-transparent shadow-none flex items-center justify-center",
      },
      width: "32rem",
  ...
  const handleFirstTimeWorkflowSave = async (formData: any) => {
    ...
    try {
      ...
      toast.success(
        "Your first workflow template has been created successfully! You can now create requests using this template.",
      );
      ...
      toast.error(
        error.response?.data?.error ||
          error.message ||
          "Failed to create workflow template. Please try again.",
      );
    }
  };
  ...
  // When on /my-notices, navigating to another section must go to /home so URL and sidebar stay in sync
  const ensureHomeRoute = () => {
    if (location.pathname.startsWith("/my-notices")) {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      label: "My Notices",
      onClick: () => {
        setSelectedSection("noticeManagementSystem");
        navigate("/my-notices");
      },
      active: selectedSection === "noticeManagementSystem" || isMyNoticesPath,
    },
    ...
    ...(user?.roles?.some((role: any) => role.id === 1 || role.id === 6) ||
    user?.role === "1" ||
    user?.role === "6"
      ? [
          {
  ...
  const [showFirstTimeWorkflowModal, setShowFirstTimeWorkflowModal] =
    useState(false);
  const [hasCheckedForExistingTemplates, setHasCheckedForExistingTemplates] =
    useState(false);

  // Account creator invite modal (for account creators on first login)
  const [showAccountCreatorInviteModal, setShowAccountCreatorInviteModal] =
    useState(false);
  const [hasCheckedAccountCreatorInvite, setHasCheckedAccountCreatorInvite] =
    useState(false);
  ...
  const requestColumns = [
    ...
    },
    {
      ...
      cell: (row: Request) => {
        ...
        return (
          <span
            className={`px-1.5 py-0.5 rounded-full text-xs font-semibold ${statusColor} inline-block whitespace-nowrap`}
          >
            {statusText}
          </span>
        );
      ...
      if (result.isConfirmed) {
        ...
        for (const request of selectedRows) {
          try {
            ...
            console.error(
              `Error processing request ${request.REQUEST_ID}:`,
              error,
            );
            errorCount++;
          }
  ...
  const handleDeleteRequests = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        for (const request of selectedRows) {
          try {
            await axios.delete(`/api/requests/${request.REQUEST_ID}`, {
              headers: {
                Authorization: `Bearer ${authToken}`,
                "Content-Type": "application/json",
              },
            ...
            console.error(
              `Error deleting request ${request.REQUEST_ID}:`,
              error,
            );
            errorCount++;
          }
  ...
  const fetchAssignedRequestsCount = async () => {
    try {
      ...
      const activeCount = assignedRequests.filter(
        (req) => req.STATUS && !["D", "X"].includes(req.STATUS),
      ).length;
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      if (Array.isArray(requestsData)) {
        if (process.env.NODE_ENV === "development") {
          console.log(
            "Successfully received array data, length:",
            requestsData.length,
          );
        }
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <header className="fixed top-0 left-0 w-full h-16 bg-white shadow-md border-b-2 border-teal-500 flex items-center justify-between px-4 md:px-8 z-40">
        <div className="flex items-center gap-2 md:gap-3">
          <img
            src="/images/GuardianLogo.svg"
            alt="Guardian Logo"
            className="h-8 w-auto"
          />
          <span className="font-bold text-lg md:text-2xl text-gray-700 hidden sm:inline">
            Guardian
          </span>
        </div>
        ...
        >
          ...
          <div
            className="flex items-center gap-2 md:gap-3 relative cursor-pointer border border-gray-200 rounded-lg px-2 mr-4"
          ...
          >
            ...
            <span className="flex flex-col items-start hidden sm:inline">
              ...
              {user &&
                ((user.roles &&
                  user.roles.some(
                    (role: any) => role.id === 1 || role.id === 6,
                  )) ||
                  user.role === "1" ||
                  user.role === "6") && (
                  <span className="text-sm text-gray-500 font-medium text-end">
                    {getUserRole()}
                  </span>
                )}
            </span>
          ...
          {profileMenuOpen && (
            <div className="absolute right-0 top-12 mt-2 w-56 bg-white rounded-lg shadow-sm border-t-4 border-t-secondary py-2 z-50 border border-gray-100 animate-fade-in">
              ...
              <button
                className="w-full flex items-center gap-2 text-left px-4 py-2 hover:bg-gray-100 text-red-600 text-sm"
      ...
      <nav
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(255, 255, 255, 0.95) 0%, rgba(248, 250, 252, 0.9) 50%, rgba(46, 188, 188, 0.05) 100%)",
          backdropFilter: "blur(20px) saturate(180%)",
          WebkitBackdropFilter: "blur(20px) saturate(180%)",
          borderRight: "1px solid rgba(189, 189, 189, 0.3)",
          boxShadow:
            "4px 0 32px rgba(3, 36, 36, 0.15), 0 0 0 1px rgba(255, 255, 255, 0.5) inset",
        }}
      >
        ...
        <div className="flex items-center justify-between px-4 py-6 border-b border-gray-4/40 bg-gradient-to-r from-secondary/5 to-transparent">
          <button
            ...
            style={{
              boxShadow:
                "0 4px 16px rgba(0, 0, 0, 0.08), 0 0 0 1px rgba(255, 255, 255, 0.8) inset",
            }}
          >
        ...
        <div
          className="flex flex-col px-3 py-6 flex-1 space-y-2"
          role="menu"
        ...
        >
          {navItems.map((item, index) => (
            <button
              ...
              style={{
                ...(item.label === "Invites" || item.label === "Assignments"
                  ? { display: "none" }
                  : {}),
                ...(item.active
                  ? {
                      background:
                        "linear-gradient(135deg, rgba(46, 188, 188, 0.12) 0%, rgba(3, 36, 36, 0.08) 100%)",
                      boxShadow:
                        "0 8px 32px rgba(46, 188, 188, 0.15), 0 0 0 1px rgba(46, 188, 188, 0.2) inset",
                    }
                  : {}),
              }}
            >
              ...
              <div
                ...
                style={
                  item.active
                    ? {
                        background:
                          "linear-gradient(135deg, rgba(46, 188, 188, 0.15) 0%, rgba(3, 36, 36, 0.10) 100%)",
              ...
              >
                ...
                <div
                  ...
                  bg-gradient-to-br from-secondary to-primary blur-sm -z-10`}
                ></div>
              </div>
              ...
              {item.badge && (
                <div
                  className={`absolute ${isNavExpanded ? "top-1 right-1" : "top-0 right-0"} 
                  ...
                  style={{
                    ...
                    boxShadow:
                      "0 4px 16px rgba(239, 68, 68, 0.4), 0 0 0 2px rgba(255, 255, 255, 0.9)",
                  }}
                >
        ...
        <div className="px-3 py-6 border-t border-gray-4/40 bg-gradient-to-r from-gray-5/20 to-transparent">
          ...
          >
            ...
            >
              ...
              {/* Subtle glow effect */}
              <div
                className="absolute inset-0 rounded-xl transition-opacity duration-300 opacity-0 group-hover:opacity-10
                bg-gradient-to-br from-red-400 to-pink-400 blur-sm -z-10"
              ></div>
            </div>
        ...
        <Tooltip
          ...
          style={{
            ...
            backdropFilter: "blur(20px) saturate(180%)",
            boxShadow:
              "0 10px 40px rgba(0, 0, 0, 0.2), 0 0 0 1px rgba(255, 255, 255, 0.1) inset",
          }}
      ...
      <main
        className={`flex-1 flex flex-col ${selectedSection === "noticeManagementSystem" || isMyNoticesPath ? "mt-[34px]" : "mt-16"} px-2 sm:px-4 md:px-8 py-4 md:py-8 pb-20 sm:pb-8 gap-4 sm:gap-6 md:gap-8 overflow-y-auto ${isNavExpanded ? "sm:ml-64" : "sm:ml-16"} transition-all duration-500 ease-out bg-gray-50 min-h-[calc(100vh-64px)]`}
      >
        {mobileNav === "dashboard" && selectedSection === "dashboard" ? (
          // Dashboard Overview
          <div className="container max-w-full">
            <h1 className="text-2xl font-bold uppercase fs-2 mb-4 sm:mb-8">
              HOME
            </h1>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
              {/* Request Overview Card */}
              <section
                className={`${theme === "dark" ? "bg-gray-800 text-white" : "bg-white"} rounded-xl shadow-sm border-t-4 border-t-secondary p-4 sm:p-6 flex flex-col h-64 sm:h-80 md:h-96 md:col-span-1 border border-gray-200`}
                data-component-name="Home"
              >
                <h2 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-center flex-shrink-0">
                  Request Overview
                </h2>
                <div className="flex flex-col items-center justify-center flex-1 min-h-0">
                  <div
                    className="w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 flex items-center justify-center relative flex-shrink-0"
                    data-component-name="Home"
                  >
                    {loading ? (
                      ...
                      <>
                        <Pie
                          data={{
                            ...
                            datasets: [
                              {
                                data: requestStatusData.map(
                                  (item) => item.value,
                                ),
                                backgroundColor: requestStatusData.map(
                                  (item) => item.color,
                                ),
                                borderWidth: 1,
                              },
              ...
              {/* Request Queue Card */}
              <section
                className={`${theme === "dark" ? "bg-gray-800 text-white" : "bg-white"} rounded-xl shadow-sm border-t-4 border-t-secondary p-4 sm:p-6 w-full md:col-span-3 border border-gray-200`}
                data-component-name="Home"
              >
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (searchTerm) {
                              const filteredData = requests.filter((item) => {
                                return (
                                  item.REQUEST_NAME?.toLowerCase().includes(
                                    searchTerm,
                                  ) ||
                                  item.TRACKINGID?.toLowerCase().includes(
                                    searchTerm,
                                  ) ||
              ...
              {/* My Notices Card */}
              <section
                className={`${theme === "dark" ? "bg-gray-800 text-white" : "bg-white"} rounded-xl shadow-sm border-t-4 border-t-blue-500 p-4 sm:p-6 w-full md:col-span-4 border border-gray-200 mt-4`}
                data-component-name="Home"
              >
                  ...
                  <div className="space-y-3">
                    {notices.slice(0, 5).map((notice) => (
                      <div
                        ...
                        onClick={() =>
                          window.open(`/notices/${notice.NOTICE_ID}`, "_blank")
                        }
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-4 text-xs text-gray-500">
                              <span>
                                By: {notice.ISSUED_BY_USER?.FIRST_NAME}{" "}
                                {notice.ISSUED_BY_USER?.LAST_NAME}
                              </span>
                              <span>
                                {new Date(
                                  notice.ISSUE_DATE || notice.CREATE_DATE,
                                ).toLocaleDateString()}
                              </span>
                              <span
            ...
            {process.env.NODE_ENV === "development" && (
              <div className="flex flex-col gap-2 text-sm">
                ...
                <div className="text-xs text-gray-600">
                  Modal State:{" "}
                  {showFirstTimeWorkflowModal ? "SHOWING" : "HIDDEN"}
                </div>
                <div className="text-xs text-gray-600">
                  Has Checked Templates:{" "}
                  {hasCheckedForExistingTemplates ? "YES" : "NO"}
                </div>
                <div className="text-xs text-gray-600">
                  User ID: {user?.id || "N/A"}
                </div>
        ...
        ) : selectedSection === "noticeManagementSystem" || isMyNoticesPath ? (
          ...
          <div className="mt-4 md:mt-6 mb-6">
            <AdminDashboard
              onShowUserManagement={() =>
                setSelectedSection("adminUserManagement")
              }
            />
          </div>
      ...
      <MobileNavBar
        ...
        onSelect={(key) => {
          if (
            ["dashboard", "search", "notifications", "profile"].includes(key)
          ) {
            setMobileNav(
              key as "dashboard" | "search" | "notifications" | "profile",
            );
            if (key === "dashboard") {
              ensureHomeRoute();
              setSelectedSection("dashboard");
